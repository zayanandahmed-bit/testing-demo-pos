...
begin
  if payload ? 'items_td' then
    ...
    from jsonb_array_elements(payload->'items_td') x;
  end if;
  ...
  if payload ? 'categories_td' then
    delete from categories_td where true;
    insert into categories_td (name)
    select jsonb_array_elements_text(payload->'categories_td');
  end if;

  if payload ? 'suppliers_td' then
    ...
    from jsonb_array_elements(payload->'suppliers_td') x;
  end if;

  if payload ? 'cashiers_td' then
    delete from cashiers_td where true;
    insert into cashiers_td (id, name, pin)
    select x->>'id', x->>'name', coalesce(x->>'pin','')
    from jsonb_array_elements(payload->'cashiers_td') x;
  end if;

  if payload ? 'purchases_td' then
    ...
    select x->>'id',
           ...
           x->>'proof', x->>'proofName'
    from jsonb_array_elements(payload->'purchases_td') x;
  end if;

  if payload ? 'expenses_td' then
    ...
    from jsonb_array_elements(payload->'expenses_td') x;
  end if;

  if payload ? 'customers_td' then
    ...
    from jsonb_array_elements(payload->'customers_td') x;
  end if;
  ...
  if payload ? 'sales_td' then
    ...
    from jsonb_array_elements(payload->'sales_td') x;
    ...
    from jsonb_array_elements(payload->'sales_td') s,
         jsonb_array_elements(coalesce(s->'lines','[]'::jsonb)) l;
  end if;

  if payload ? 'refunds_td' then
    ...
    from jsonb_array_elements(payload->'refunds_td') x;
    ...
    from jsonb_array_elements(payload->'refunds_td') r,
  ...
  if payload ? 'shifts_td' then
    ...
    from jsonb_array_elements(payload->'shifts_td') x;
  end if;
  ...
  if payload ? 'settings_td' then
    delete from settings_td where true;
    insert into settings_td (key, value)
    select k, v
    from jsonb_each_text(payload->'settings_td') as t(k, v);
  end if;
...
begin
  select jsonb_build_object(
    'items_td', coalesce((select jsonb_agg(jsonb_build_object(
    ...
    'categories_td', coalesce((select jsonb_agg(name) from categories_td), '[]'::jsonb),

    'suppliers_td', coalesce((select jsonb_agg(jsonb_build_object(
      'id', id, 'name', name, 'contact', contact, 'address', address
    )) from suppliers_td), '[]'::jsonb),

    'cashiers_td', coalesce((select jsonb_agg(jsonb_build_object(
      'id', id, 'name', name, 'pin', pin
    )) from cashiers_td), '[]'::jsonb),

    'purchases_td', coalesce((select jsonb_agg(jsonb_build_object(
    ...
    'expenses_td', coalesce((select jsonb_agg(jsonb_build_object(
      'id', id, 'date', date, 'period', period, 'category', category, 'amount', amount, 'notes', notes
    )) from expenses_td), '[]'::jsonb),

    'customers_td', coalesce((select jsonb_agg(jsonb_build_object(
    ...
    'shifts_td', coalesce((select jsonb_agg(jsonb_build_object(
    ...
    'settings_td', coalesce((select jsonb_object_agg(key, value) from settings_td), '{}'::jsonb),
...
create or replace function sync_append_sales_batch_td(sales_td jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  select x->>'id', (x->>'receiptNo')::integer, (x->>'date')::timestamptz, x->>'customer', x->>'cashier', x->>'payment',
         (x->>'cash')::numeric, (x->>'subtotal')::numeric, (x->>'discountPct')::numeric, (x->>'discountAmt')::numeric,
         (x->>'taxPct')::numeric, (x->>'taxAmt')::numeric, (x->>'grand')::numeric
  from jsonb_array_elements(sales_td) x
  on conflict (id) do nothing;

  insert into sale_lines_td (sale_id, item_id, item_name, barcode, price, qty, unit, subtotal)
  select s->>'id', l->>'itemId', l->>'name', coalesce(l->>'barcode',''), (l->>'price')::numeric,
         (l->>'qty')::numeric, coalesce(l->>'unit',''), (l->>'subtotal')::numeric
  from jsonb_array_elements(sales_td) s,
       jsonb_array_elements(coalesce(s->'lines','[]'::jsonb)) l;
end;
$$;
...
create or replace function sync_replace_items_td(items_td jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  select x->>'id', x->>'name', coalesce(x->>'barcode',''), coalesce(x->>'category',''),
         coalesce((x->>'price')::numeric,0), coalesce((x->>'cost')::numeric,0), coalesce((x->>'stock')::numeric,0),
         coalesce(x->>'unit',''), coalesce((x->>'lowStock')::numeric,0), coalesce(x->>'expiry','')
  from jsonb_array_elements(items_td) x;
end;
$$;
...
create or replace function sync_replace_categories_td(categories_td jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  delete from categories_td where true;
  insert into categories_td (name)
  select jsonb_array_elements_text(categories_td);
end;
$$;

create or replace function sync_replace_suppliers_td(suppliers_td jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  insert into suppliers_td (id, name, contact, address)
  select x->>'id', x->>'name', coalesce(x->>'contact',''), coalesce(x->>'address','')
  from jsonb_array_elements(suppliers_td) x;
end;
$$;

create or replace function sync_replace_cashiers_td(cashiers_td jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  insert into cashiers_td (id, name, pin)
  select x->>'id', x->>'name', coalesce(x->>'pin','')
  from jsonb_array_elements(cashiers_td) x;
end;
$$;

create or replace function sync_replace_purchases_td(purchases_td jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  select x->>'id',
         ...
         (x->>'qty')::numeric, (x->>'cost')::numeric, (x->>'total')::numeric, coalesce(x->>'notes',''),
         x->>'proof', x->>'proofName'
  from jsonb_array_elements(purchases_td) x;
end;
$$;

create or replace function sync_replace_expenses_td(expenses_td jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  select x->>'id', nullif(x->>'date','')::date, coalesce(x->>'period',''), coalesce(x->>'category',''),
         coalesce((x->>'amount')::numeric,0), coalesce(x->>'notes','')
  from jsonb_array_elements(expenses_td) x;
end;
$$;

create or replace function sync_replace_customers_td(customers_td jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  insert into customers_td (id, name, phone, notes, balance)
  select x->>'id', x->>'name', coalesce(x->>'phone',''), coalesce(x->>'notes',''), coalesce((x->>'balance')::numeric,0)
  from jsonb_array_elements(customers_td) x;
end;
$$;
...
create or replace function sync_replace_held_sales_td(held_sales_td jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  insert into held_sales_td (id, date, customer, discount, tax, cashier)
  select x->>'id', (x->>'date')::timestamptz, x->>'customer', (x->>'discount')::numeric, (x->>'tax')::numeric, x->>'cashier'
  from jsonb_array_elements(held_sales_td) x;

  insert into held_sales_cart_td (held_id, item_id, qty)
  select h->>'id', c->>'itemId', (c->>'qty')::numeric
  from jsonb_array_elements(held_sales_td) h,
       jsonb_array_elements(coalesce(h->'cart','[]'::jsonb)) c;
end;
$$;

create or replace function sync_replace_shifts_td(shifts_td jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  select x->>'id', x->>'cashierName', (x->>'start')::timestamptz, (x->>'end')::timestamptz,
         ...
         (x->>'cashRefunds')::numeric, (x->>'txnCount')::integer, (x->>'expectedCash')::numeric,
         (x->>'actualCash')::numeric, (x->>'difference')::numeric, coalesce(x->>'notes','')
  from jsonb_array_elements(shifts_td) x;
end;
$$;

create or replace function sync_replace_active_shift_td(active_shift_td jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  delete from active_shift_td where true;
  if active_shift_td is not null and active_shift_td != 'null'::jsonb then
    insert into active_shift_td (id, cashier_id, cashier_name, start, opening_cash)
    values (
      active_shift_td->>'id',
      active_shift_td->>'cashierId',
      active_shift_td->>'cashierName',
      (active_shift_td->>'start')::timestamptz,
      (active_shift_td->>'openingCash')::numeric
    );
  end if;
end;
$$;

create or replace function sync_replace_settings_td(settings_td jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  insert into settings_td (key, value)
  select k, v
  from jsonb_each_text(settings_td) as t(k, v);
end;
$$;
